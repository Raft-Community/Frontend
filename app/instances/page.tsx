      ...
      <div className="card bg-gray-400 shadow-xl">
        <Link href={`/home?id=${id}`} className='h-full'>
          <div className="card-body">
            ...
            <p className="card-text">Port: {Math.floor(Math.random() * 9000) + 1000}</p>
          </div>
        </Link>
      </div>
  );
...
function AddInstance() {
  return (
    <div className="card bg-gray-400 shadow-xl flex justify-center items-center">
      <button className="btn" onClick={()=>(window as any).addInstance.showModal()}>Add an instance</button>
